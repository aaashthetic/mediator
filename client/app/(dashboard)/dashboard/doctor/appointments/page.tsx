...
export default async function DoctorAppointmentsPage() {
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/api/appointments?role=doctor&userId=${userId}`, {
      method: "GET",
      headers: {
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-in fade-in duration-300">
      ...
      <Tabs defaultValue="upcoming" className="w-full space-y-6">
        ...
        <TabsContent value="upcoming" className="outline-none w-full">
          <DoctorAppointments appointments={upcomingAppointments} type="UPCOMING" viewerRole="doctor" />
        </TabsContent>

        <TabsContent value="past" className="outline-none w-full">
          <DoctorAppointments appointments={pastAppointments} type="PAST" viewerRole="doctor" />
        </TabsContent>
      </Tabs>
    </div>
  );
