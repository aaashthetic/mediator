...
export default function OnboardingClientForm({ initialPhone }: FormProps) {
  ...
  // Doctor Form
  return (
    <form onSubmit={(e) => handleOnboardingSubmit(e, 'doctor')} className="space-y-4 animate-in fade-in duration-200">
      <h2 className="text-xl font-bold text-slate-800 dark:text-zinc-100">Doctor Professional Profile</h2>
      <button type="button" disabled={isSubmitting} onClick={() => setRole(null)} className="text-xs text-emerald-500 hover:underline disabled:opacity-50">← Go Back</button>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-zinc-400 mb-1">First Name</label>
          <input required disabled={isSubmitting} type="text" name="firstName" className="w-full p-2 border rounded-md bg-transparent border-slate-200 dark:border-zinc-800 disabled:bg-slate-50" />
        </div>
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-zinc-400 mb-1">Last Name</label>
          <input required disabled={isSubmitting} type="text" name="lastName" className="w-full p-2 border rounded-md bg-transparent border-slate-200 dark:border-zinc-800 disabled:bg-slate-50" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input required disabled={isSubmitting} type="text" name="specialization" placeholder="e.g. Cardiologist, Pediatrician" className="w-full p-2 border rounded-md bg-transparent border-slate-200 dark:border-zinc-800 disabled:bg-slate-50" />
        </div>
        
        {/* 💸 Dynamic Fee Field Addition */}
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-zinc-400 mb-1">Consultation Fee (PHP)</label>
          <input required disabled={isSubmitting} type="number" min="0" step="50" name="consultationFee" defaultValue="500" className="w-full p-2 border rounded-md bg-transparent border-slate-200 dark:border-zinc-800 disabled:bg-slate-50" />
        </div>
      </div>

      {/* ⚡ Medical Practice Start Date Field Addition */}
      <div>
        <label className="block text-xs font-medium text-slate-600 dark:text-zinc-400 mb-1">Medical Practice Start Date</label>
        <input required disabled={isSubmitting} type="date" name="medicalPracticeStartDate" max={new Date().toISOString().split("T")[0]} className="w-full p-2 border rounded-md bg-transparent border-slate-200 dark:border-zinc-800 disabled:bg-slate-50" />
        <p className="text-[10px] text-muted-foreground mt-1">Used to compute your professional years of experience automatically.</p>
      </div>

      <div>
        <label className="block text-xs font-medium text-slate-600 dark:text-zinc-400 mb-1">Professional Bio</label>
        <textarea required disabled={isSubmitting} name="bio" placeholder="Brief statement about your clinical background, experience, or medical practice mission..." rows={4} className="w-full p-2 border rounded-md bg-transparent border-slate-200 dark:border-zinc-800 disabled:bg-slate-50" />
      </div>

      <div className="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-md text-amber-800 dark:text-amber-400 text-xs">
      ...
      <button type="submit" disabled={isSubmitting} className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md font-medium transition flex items-center justify-center gap-2 disabled:opacity-50 shadow-sm active:scale-[0.99]">
        {isSubmitting && <Loader2 className="h-4 w-4 animate-spin" />}
